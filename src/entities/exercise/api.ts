...
export const exerciseApi = {
  async getAll(): Promise<ApiResponse<ExerciseDto[]>> {
    const { data: response } = await apiClient.get('/exercises')
    return response
  },
  async getById(id: number): Promise<ApiResponse<ExerciseDto>> {
    const { data: response } = await apiClient.get(`/exercises/${id}`)
    return response
  },
  async add(dto: CreateExerciseDto): Promise<ApiResponse<ExerciseDto>> {
    const { data: response } = await apiClient.post('/exercises', dto)
    return response
  },
  async update(id: number, dto: UpdateExerciseDto): Promise<ApiResponse<never>> {
